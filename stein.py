...
def cam_pruning(A, X, cutoff, prune_only=True, pns=False):
    ...
    if prune_only:
        def retrieve_result():
            ...
            return A
        dag = launch_R_script("SCORE/cam_pruning.R", arguments, output_function=retrieve_result)
        return dag
    else:
        def retrieve_result():
            A = pd.read_csv(arguments['{PATH_RESULTS}']).values
            ...
            return A, Afull
        dag, dagFull = launch_R_script("SCORE/CAM.R", arguments, output_function=retrieve_result)
